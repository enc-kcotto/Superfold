...
#              |_|                                #"""
    ...
    if not debug:
        partitionPairing = generateAndRunPartition(
            args.mapObj,
            args.allConstraints,
            args.partitionWindowSize,
            args.partitionStepSize,
            args.safeName,
            args.SHAPEslope,
            args.SHAPEintercept,
            args.np,
    ...
    initialStructure = CT()
    if not debug:
        initialStructure = generateAndRunFold(
            args.mapObj,
            args.allConstraints,
            dsConstraint,
            args.foldWindowSize,
    ...
    finalStructure = CT()
    finalStructure.pair2CT(
        nonPKpairs + pkPair, seq=initialStructure.seq, name="finalStructure wPKs"
    )
    ...
    lowSHAPEregions = mainShannonFunc(
        args.mapObj.origSHAPE, bpShannonEntropy, shannonShapeName, finalStructure
    )

    # plot the arcs along with the shannon/shape reactivities
    arcFileName = "{0}/arcPlot_{1}.pdf".format(resultsDir, args.safeName)
    ensembleRNA_splitPlot(
        partitionPairing, initialStructure, pk=pkPair, outFile=arcFileName
    )
    ...
    for i, j in lowSHAPEregions:
        ...
        ct_name = "{2}/regions/region_{3}_{0:0>{maxChar}}_{1:0>{maxChar}}.ct".format(
            i, j, resultsDir, args.safeName, maxChar=maxChar
        )
        ps_name = "{2}/regions/region_{3}_{0:0>{maxChar}}_{1:0>{maxChar}}.ps".format(
            i, j, resultsDir, args.safeName, maxChar=maxChar
        )
        pvclient_name = "{2}/regions/region_{3}_{0:0>{maxChar}}_{1:0>{maxChar}}".format(
            i, j, resultsDir, args.safeName, maxChar=maxChar
        )
        ...
        # file lines
        lines = makeCircle(
            x, x, tmpZeros, tmpSHAPE, {"i": [], "j": [], "correl": []}, [], offset=i
        )
...
def generateAndRunFold(
    ...
    if rnaLength - windowSize < 200:
        ...
        foldCMD = "Fold {0}.seq {0}.ct -sh {0}.shape -sm {1} -si {2} -md {3} -C {0}.const -m 100 -w 0".format(
            fname, shapeSlope, shapeIntercept, maxDist
        )
    ...
    targetFolderRNAs, baseCount = MasterModel_readAndRenumberAll(
        targetRNA, dirname, prefix
    )
...
def generateAndRunPartition(
    ...
    if rnaLength - windowSize < 200:
        ...
        foldCMD = "partition {0}.seq {0}.pfs -sh {0}.shape -sm {1} -si {2} -md {3} -C {0}.const".format(
            fname, shapeSlope, shapeIntercept, maxDist
        )
...
def parseArgs():
    ...
    arg = argparse.ArgumentParser(
        description="SuperFold takes a windowing approach to break up the folding of large RNAs. Dividing the folding of a large RNA into smaller segments allows modern multi-core workstations to model RNA structures in a modest amount of clock-time. See README file for further details and file descriptions",
        epilog="SuperFold v1.0 by Gregg Rice ( gmr@unc.edu )",
    )
    arg.add_argument(
        "mapFile",
        type=str,
        help="SHAPE-MaP .map file, trimmed to fold desired sequence",
    )
    arg.add_argument(
        "--ssRegion", type=str, help="file containing forced single stranded regions"
    )
    arg.add_argument(
        "--pkRegion",
        type=str,
        help="text file containing pseudoknotted basepairs:\npaired nucleotides on each line seperated by whitespace. e.g.\n\n1 50(newline)2 49(newline)...",
    )
    arg.add_argument(
        "--np", type=int, default=2, help="number of processors to use, default:2"
    )
    arg.add_argument(
        "--SHAPEslope",
        type=float,
        default=1.8,
        help="SHAPE pseudofreeenergy slope, default:1.8",
    )
    arg.add_argument(
        "--SHAPEintercept",
        type=float,
        default=-0.6,
        help="SHAPE pseudofreeenergy intercept, default:-0.6",
    )
    arg.add_argument(
        "--differentialFile",
        type=str,
        help=".mapd file containing NMIA-1M6 calculated values",
    )
    arg.add_argument(
        "--differentialSlope",
        type=float,
        default=2.1,
        help="differential SHAPE slope, default:2.1",
    )
    arg.add_argument(
        "--trimInterior",
        type=int,
        default=300,
        help="number of nucleotides to trim to improve negative end effects during windowed folding, default:300",
    )
    arg.add_argument(
        "--partitionWindowSize",
        type=int,
        default=1200,
        help="length of the partition window size, default:1200",
    )
    arg.add_argument(
        "--partitionStepSize",
        type=int,
        default=100,
        help="spacing between partition windows, default:100",
    )
    arg.add_argument(
        "--foldWindowSize",
        type=int,
        default=3000,
        help="length of the Fold window size, default:3000",
    )
    arg.add_argument(
        "--foldStepSize",
        type=int,
        default=300,
        help="spacing between Fold windows, default:300",
    )
    arg.add_argument(
        "--maxPairingDist",
        type=int,
        default=600,
        help="Maximum pairing distance for partition and Fold, default:600",
    )
    arg.add_argument(
        "--noPVclient",
        action="store_false",
        help="Don't draw secondary structures using PVclient",
    )
    ...
    if o.differentialFile:
        ...
        calcDiffEnergy = differentialEnergy(
            o.diffMapFile.shape, o.differentialSlope, 0, o.diffMapFile.zfactor
        )

        calcFakeShape = fakeSHAPE(
            calcSHAPEenergy, calcDiffEnergy, o.SHAPEslope, o.SHAPEintercept
        )
...
def mainAssemble(folderPath, trim=300):
    ...
    for dpKey in list(targetDP.keys()):
        ...
        finalDP.dp["logBP"] = np.append(
            finalDP.dp["logBP"], targetDP[dpKey].dp["logBP"]
        )
...
def concatonateDP(dpObj, coverage):
    ...
    for i, j in pairs:
        ...
        # outObj.dp['logBP'] = np.append(outObj.dp['logBP'], dp['logBP'][dpFilter] )
            ...
            outObj.dp["logBP"][n] = (
                np.sum(dp["logBP"][dpFilter]) / outObj.dp["coverage"][n]
            )
...
def progress(num, outof):
    ...
    meter = "".join(["="] * int(num / outof * width)) + "".join(
        [" "] * int((outof - num) / outof * width)
    )
    line += meter[: width - 4]
    line += "]"
...
def mainShannonFunc(shapeReact, shannonEntropy, saveName, ctStruct):
    ...
    lowShannonLowSHAPE = plt.fill_between(
        list(range(len(b))),
        b * 3.0,
        0,
        alpha=0.2,
        color="blue",
        label="Low Shannon low SHAPE",
    )
    ...
    shannonPlot = plt.fill_between(
        x,
        shannon,
        np.zeros_like(shannon),
        facecolor="brown",
        interpolate=True,
        label="Shannon",
    )
    ...
    plt.ylim(-1, 1)
    plt.legend(
        [lowShannonLowSHAPE, shapePlot, shannonPlot],
        ["Low SHAPE low Shannon", "SHAPE", "Shannon"],
        loc=2,
    )
